...
class GUI_class:
    ...
    # --------------------- INIT ----------------------------
    waypoint_count, yaw, yaw_count, yaw_count_last, runMode = 0,0,0,0,0
    ...
    def __init__(self):
        ...
        with open('config.json', 'r') as file:
            ...
            # ---- parameters -------------------------
            self.VECTOR_obj = VECTOR.VECTOR_class(waypoints=self.waypoints,lat_ref=config['ref_lat'],lon_ref=config['ref_lon'])
        ...
        tk.Label(self.mqttframe,textvariable=self.WVAR_imu).pack(side=tk.LEFT)
        ...
        tk.Button(self.wpframe, text="Go to waypoints", command=self.WayPoints).pack(side=tk.LEFT)
    ...
    def loop_Updates(self):
        ...
        if self.is_Simulation.get():   # if simulation
            ...
            self.WVAR_imu.set("IMU: Yaw=" + str(self.yaw) + ", Count=" + str(self.yaw_count))
        # Current location
        x,y = self.VECTOR_obj.X, self.VECTOR_obj.Y # self.VECTOR_obj.get_X_Y(lat=self.VECTOR_obj.gps_obj.lat_ref, lon=self.VECTOR_obj.gps_obj.lon_ref)
        self.draw_Arrow_Location_Meters(angle=self.yaw, X=x, Y=y)

        # Next Waypoint
        wplat,wplon = self.waypoints[self.waypoint_count]
        ...
        self.root.after(1000, self.loop_Updates)        # loop_Updates

    # ----------------------- WayPoints -----------------------------------
    def WayPoints(self):
        self.VECTOR_obj.WayPoint_Go(index=0, speed=self.entrySpeed.get())
    #    print("Waypoints")
